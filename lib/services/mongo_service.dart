...
class MongoService {
  ...
  static Future<List<Episode>> getEpisodesWithPersonnages() async {
    ...
    for (var ep in episodesRaw) {
      final epId = ep['_id'];
      final personnagesRaw = await personnagesCollection.find(where.eq('episodeId', epId)).toList();
      final personnages = personnagesRaw.map((p) => Personnage.fromMap(p)).toList();
